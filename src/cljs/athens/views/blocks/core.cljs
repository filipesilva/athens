           ...
           [:> Anchor {:isClosedWithChildren (when (and (seq children)
                       ...
                       :menuActions (clj->js [{:children
                                               (if (> (count selected-items) 1)
                                                 ...
                                                 "Copy block ref")
                                               :onClick #(context-menu/handle-copy-refs nil uid)}
                                              {:children "Copy unformatted text"
                                               :onClick #(context-menu/handle-copy-unformatted uid)}
                                              (when (empty? selected-items)
                                                {:children "Comment"
                                                 :onClick #(context-menu/handle-click-comment uid)})
                                              (when (some #{current-page} bot/athens-users)
                                                {:children "Move notifications to read"
                                                 :onClick #(context-menu/handle-move-notifications current-page uid)})])
                       :onClick        (fn [e]
                                         (let [shift? (.-shiftKey e)]
                                           (rf/dispatch [:reporting/navigation {:source :block-bullet
                                                                                :target :block
